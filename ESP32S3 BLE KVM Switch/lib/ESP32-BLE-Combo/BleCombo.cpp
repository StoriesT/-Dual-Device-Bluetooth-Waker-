...
#define SYSTEM_CONTROL_ID 0x04
#define SYSTEM_SLEEP 0x02 // bit 1 of the System Control report = System Sleep
...
static const uint8_t _hidReportDescriptor[] = {
  ...
  COLLECTION(1),       0x01,         // COLLECTION (Application)
  REPORT_ID(1),        SYSTEM_CONTROL_ID, // REPORT_ID (4)
  REPORT_SIZE(1),      0x01,         //   REPORT_SIZE (1)
  REPORT_COUNT(1),     0x03,         //   REPORT_COUNT (3)
...
void BleCombo::begin(void)
{
  ...
  pServer->advertiseOnDisconnect(false); // don't auto-restart advertising
  ...
  inputSystemControl = hid->inputReport(SYSTEM_CONTROL_ID);
...
void BleCombo::sleep(void)
{
  if (this->isConnected())
  {
    uint8_t press = SYSTEM_SLEEP;
    this->inputSystemControl->setValue(&press, 1);
    this->inputSystemControl->notify();
    this->delay_ms(50);
    uint8_t release = 0x00;
    this->inputSystemControl->setValue(&release, 1);
    this->inputSystemControl->notify();
  }
}

void BleCombo::startAdvertising(void)
{
  if (advertising)
  {
    advertising->stop();
...
void BleCombo::startAdvertisingDirected(const uint8_t *mac)
{
  if (advertising)
  {
    advertising->stop();
    ...
    advertising->start(0, nullptr, &nimAddr);
  }
}

void BleCombo::stopAdvertising(void)
{
  if (advertising)
    advertising->stop();
}
...
void BleCombo::onConnect(BLEServer* pServer, ble_gap_conn_desc* desc) {
  if (desc) {
    memcpy(_peerMac, desc->peer_id_addr.val, 6);
    _hasPeerMac = true;
    Serial.printf("[BLE] peer MAC: %02X:%02X:%02X:%02X:%02X:%02X\n",
